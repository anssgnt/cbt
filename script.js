...
async function gasRun(funcName, ...args) {
  ...
  try {
    if (funcName === 'getAllPeserta') {
      ...
      const data = snap.val() || {};
      const results = [];
      for (let id in data) results.push({ id, name: data[id].nama, kelas: data[id].kelas });
      return results;
    }
